...
class Parser:
    ...
    async def extract_living(self, text):
        ...
        for span in doc.spans:
            if span.type in ['LOC', 'GPE']:  # LOC для локаций, GPE для геополитических объектов (города, страны)
    ...
    async def extract_work(self, text):
        ...
        # Извлечение и форматирование информации о местах работы
        work_experiences = []

        for match in work_pattern.finditer(resume_text):
            ...
            work_experiences.append(f"{company} {start_month}.{start_year} - {end_date}, {position}")
